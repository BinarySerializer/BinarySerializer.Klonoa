namespace BinarySerializer.Klonoa.LV {
    public class SFZ_File : BaseFile
    {
        // This is needed to get the vertices/normals pointers from the subparts of the target mesh
        ...
        public override void SerializeImpl(SerializerObject s) 
        {
            if (!IsEmpty)
            {
                ...
                SFXPart mesh = Pre_Parts[TargetMesh];
                Vertices ??= new KlonoaLV_Vector16[mesh.SubpartCount][];
                Normals ??= new KlonoaLV_Vector16[mesh.SubpartCount][];

                for (int i = 0; i < mesh.SubpartCount; i++)
                {
                    SFXSubpart skinDescriptor = mesh.Subparts[i];
