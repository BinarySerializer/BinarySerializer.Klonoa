namespace BinarySerializer.Klonoa.LV
{
    public class CameraData_File : BaseFile
    {
        ...
        public RouteCameraData[][] RouteData { get; set; }
        ...
        public override void SerializeImpl(SerializerObject s)
        {
            ...
            RouteData ??= new RouteCameraData[Count][];
            for (int i = 0; i < Count; i++)
                RouteData[i] = s.SerializeObjectArrayUntil<RouteCameraData>(RouteData[i], (c) => (c.Flag & 64) != 0, name: $"{nameof(RouteData)}[{i}]");
